...
public class WritingHandler : MonoBehaviour
{
		...
		//if (hand != null) {
		if (DataLevel.Instance.CameraLetters) {
		...
		private void EndTouchLetterHandle ()
		{
				...
				if (!clickBeganOrMovedOutOfLetterArea) {
					...
					if(part.priority==1)
					{
						DataLevel.Instance.SetEndPart1P ();
					}
										break;
								}
						}
		...
		private void CheckLetterDone ()
		{
			...
			//hand = null;
			Debug.Log ("finished2");
			DataLevel.Instance.ChangedSate(Mark.finished_write_letter);
